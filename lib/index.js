...
module.exports = class TCPTransport extends Transport {
  constructor (options) {
    ...
    assert(this.options.streamName, 'Must supply a streamName')
    assert(this.options.sourceName, 'Must supply a sourceName')

    // generic transport requirements
    this.name = 'winston-logio'
  ...
  write (info, callback) {
    ...
    if (this.connected) {
      ...
      this.socket.write("+msg|"+this.options.streamName+"|"+this.options.sourceName+"|"+info.level+": "+info.message+"\0", 'utf8', () => {
        /* istanbul ignore if */
